...
Goblin.registerQuest(goblinName, 'search', function*(
  ...
  if (value) {
    ...
    /*const queries = filters.reduce((queries, filter) => {
      queries.push(bob.termsQuery(filter.name, filter.value));
      return queries;
    }, []);*/

    //body = body.query(bob.disMaxQuery().queries(queries));
    body = body.query(
      bob.boolQuery().mustNot(bob.termsQuery(filters[0].name, filters[0].value))
    );
    if (sort) {
      body = body.sort(bob.sort(sort.key, sort.dir));
    }
