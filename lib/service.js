...
Goblin.registerQuest(goblinName, 'search', function* (
  ...
  if (!fullTextFields) {
    fullTextFields = useQuote
      ? ['info']
      : ['searchPhonetic', 'searchAutocomplete'];
  }
  ...
  if (value) {
    /* If at least one word is surrounded by ["], set the Levenshtein distance to 0 (exact) */
    if (!fuzzy && useQuote) {
      fuzzValue = '0';
    }
    ...
    switch (searchMode) {
      ...
      case 'fulltext':
        ...
        if (value.length > 0) {
          body = body.query(
            useQuote
              ? bob.matchPhraseQuery(fullTextFields[0], value)
              : bob
  ...
  if (searchAfter && sort) {
    ...
    body = body.searchAfter(
      onlyUid ? [searchAfter[searchAfter.length - 1]] : searchAfter
    );
  }
...
Goblin.registerQuest(goblinName, 'bulk', function* (
  ...
  try {
    const client = quest.goblin.getX('client');
    indexed = yield client.bulk({body}, next);
  } finally {
    sem.signal();
  }
  if (withInfo) {
