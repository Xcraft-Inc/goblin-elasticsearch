...
Goblin.registerQuest (goblinName, 'index', function* (
  ...
  const client = quest.goblin.getX ('client');
  const index = quest.goblin.getX ('index');
  const indexed = yield client.index ({index, type, id, body: document}, next);
  quest.log.info (indexed);
});

Goblin.registerQuest (goblinName, 'unindex', function* (quest, type, id, next) {
  const client = quest.goblin.getX ('client');
  const index = quest.goblin.getX ('index');
  const deleted = yield client.index ({index, type, id}, next);
