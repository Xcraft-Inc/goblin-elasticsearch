  ...
  Goblin.registerQuest(goblinName, 'search', function* (
    ...
    if (workshopConfig.enableMultiLanguageIndex) {
      ...
      if (currentLocaleName) {
        elastic = quest.getStorage(
          'elastic',
          `${mandate}-${currentLocaleName}`
        );
      }
    }

    const hinterId = quest.goblin.getX('hinterId');
    const hinter = quest.getAPI(hinterId);
    ...
    if (results) {
      const rows = results.hits.hits.map((hit) => {
        ...
        if (hit.highlight.info) {
          info = true;
        }

        if (!phonetic && !autocomplete && !info) {
          return hit._source.info;
        }
        ...
        let field;
        if (phonetic) {
          field = hit.highlight.searchPhonetic[0];
        } else if (autocomplete) {
          field = hit.highlight.searchAutocomplete[0];
        } else if (info) {
          field = hit.highlight.info[0];
        }
        return MarkdownBuilder.emToBacktick(field);
      });
