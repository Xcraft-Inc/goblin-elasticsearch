...
function hinterFactory(config) {
  ...
  Goblin.registerQuest(goblinName, 'create', function*(
    ...
    const currentLocale = yield deskAPI.getCurrentLocale();

    quest.goblin.setX(
      'currentLocale',
      currentLocale ? currentLocale.toLowerCase().replace(/\//g, '-') : null
    );
  ...
  Goblin.registerQuest(goblinName, 'search', function*(quest, value) {
    ...
    if (currentLocale) {
      elastic = quest.getStorage('elastic', `${mandate}-${currentLocale}`);
    }
    ...
    if (results) {
      ...
      const status = results.hits.hits.map(hit => hit._source.status);
